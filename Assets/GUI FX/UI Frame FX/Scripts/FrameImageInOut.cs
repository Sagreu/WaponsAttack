...
namespace GUIFX
{
    public class FrameImageInOut : MaskableGraphic
    {
        [SerializeField]
        ...
        protected override void OnEnable()
        {
            ...
            // 确保纹理的 Wrap Mode 为 Clamp
            if (m_InstancedMaterial.mainTexture != null)
            {
                m_InstancedMaterial.mainTexture.wrapMode = TextureWrapMode.Clamp;
            }
            if (m_Sprite != null && m_Sprite.texture != null)
            {
                // 修改纹理的 wrapMode 为 Clamp
        ...
        private void UpdateUVOffset()
        {
            ...
            // 更新UV偏移
            m_UVOffset += m_UVSpeed * 0.03f; // 0.03秒是更新间隔
            m_UVOffset = m_UVOffset > 1 ? -1 : m_UVOffset; // 保持在-1到1之间
            SetVerticesDirty(); // 强制更新网格
        }
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            for (int i = 0; i <= m_SegmentCount; i++)
            {
                ...
                float uvX = (float)i / m_SegmentCount - 0.03f + m_UVOffset; // 应用UV偏移

                verts[i * 2] = CreateVertex(outerPos, new Vector2(uvX, 0));
                verts[i * 2 + 1] = CreateVertex(innerPos, new Vector2(uvX, 1));
            }
